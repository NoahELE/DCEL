/* Compile with
gcc `pkg-config --cflags gtk+-3.0` -o gtk gtk.c `pkg-config --libs gtk+-3.0` -lm
-g `pkg-config --static --libs --cflags igraph`
*/
...
void setupGTKBounds(double newxmin, double newxmax, double newymin,
                    double newymax) {
    minx = newxmin;
    miny = newymin;
    maxx = newxmax;
...
void setGTKWatchTower(int vertex) { selectedWT = vertex; }

void setGTKVertex(int vertex) { selectedPolyPoint = vertex; }

void setGTKEdge(int edge) { selectedPolyEdge = edge; }

void setupGTKPoints(int pointCount, void *displayData,
                    char *(*getDataString)(void *, int),
                    void (*freeDataString)(char *),
                    double (*getPointx)(int, void *),
...
void setupGTKPolyPoints(int dcelVerticesCount, void *dcel,
                        double (*getDCELVertexX)(void *, int),
                        double (*getDCELVertexY)(void *, int)) {
    polygonsPointsSet = 1;
...
void setupGTKPolyLines(int edgeCount, void *dcel,
                       ...
                       int (*getDCELEdgeVertexPairEnd)(void *, int),
                       int (*DCELhasEdge)(void *, int),
...
void setupGTKAction(void (*doAction)(void *, double, double),
                    void *actionContext) {
    actionSet = 1;
...
static gboolean on_draw(GtkWidget *widget, cairo_t *cr, gpointer user_data) {
    ...
    void drawLine(gdouble x_from, gdouble y_from, gdouble x_to, gdouble y_to,
                  int edge, int sVertex, int eVertex) {
        ...
        // cairo_move_to (cr, x_to + 0.05 * cos(angle), y_to + 0.05 *
        // sin(angle)); cairo_line_to (cr, x2, y2); cairo_stroke(cr);
    }
    if (polygonLinesSet) {
        if (!polygonsPointsSet) {
        ...
        for (j = 0; j < gtkedgeCount; j++) {
            ...
            xStart =
                ((gtkgetDCELVertexX(gtkdcel, startVertex) - avgX) / rangeX) * 5;
            yStart =
                (-(gtkgetDCELVertexY(gtkdcel, startVertex) - avgY) / rangeY) *
                5;
            xEnd =
                ((gtkgetDCELVertexX(gtkdcel, endVertex) - avgX) / rangeX) * 5;
            yEnd =
                (-(gtkgetDCELVertexY(gtkdcel, endVertex) - avgY) / rangeY) * 5;
            ...
            if (pair) {
                ...
                pairxStart =
                    ((gtkgetDCELVertexX(gtkdcel, pairStartVertex) - avgX) /
                     rangeX) *
                    5;
                pairyStart =
                    (-(gtkgetDCELVertexY(gtkdcel, pairStartVertex) - avgY) /
                     rangeY) *
                    5;
                pairxEnd = ((gtkgetDCELVertexX(gtkdcel, pairEndVertex) - avgX) /
                            rangeX) *
                           5;
                ;
                pairyEnd =
                    (-(gtkgetDCELVertexY(gtkdcel, pairEndVertex) - avgY) /
                     rangeY) *
                    5;
            }
...
static gboolean mouse_moved(GtkWidget *widget, GdkEvent *event,
                            gpointer user_data) {
    ...
    if (event->type == GDK_MOTION_NOTIFY) {
        ...
        /* Determine GtkDrawingArea dimensions */
        gdk_window_get_geometry(window, &da.x, &da.y, &da.width, &da.height);
        // printf("%f %f %d %d\n", (e->x - da.width / 2) / dx, (e->y - da.height
        // / 2) / dy, da.width, da.height); Scale to Cairo zoom level
...
static gboolean button_pressed(GtkWidget *widget, GdkEvent *event,
                               gpointer user_data) {
    if (actionSet) {
