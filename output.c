...
void outputResult(char *outputfileName, struct watchtowerStruct **wts,
                  int wtCount, struct DCEL *dcel) {
    ...
    if (!dcel) {
        ...
        for (i = 0; i < wtCount; i++) {
            ...
            fprintf(outputfile,
                    ...
                    (wts[i])->watchtowerID, (wts[i])->postcode,
                    (wts[i])->populationServed, (wts[i])->contact, (wts[i])->x,
                    (wts[i])->y);
            population += wts[i]->populationServed;
        }
        fprintf(outputfile, "Face undefined population served: %d\n",
                population);
    } else {
        ...
        for (i = 0; i < (faceCount + 1); i++) {
            ...
            if (i != faceCount) {
                ...
                for (j = 0; j < faceSize[i]; j++) {
                    fprintf(
                        outputfile,
                        "Watchtower ID: %s, Postcode: %s, Population Served: "
                        "%d, "
                        "Watchtower Point of Contact Name: %s, x: %f, y: %f\n",
                        ...
                        (wts[faceData[i][j]])->x, (wts[faceData[i][j]])->y);
                    populationTotals[i] +=
                        wts[faceData[i][j]]->populationServed;
                }
            }
...
void outputResultDiameter(char *outputfileName, struct watchtowerStruct **wts,
                          int wtCount, struct DCEL *dcel) {
    FILE *outputfile = fopen(outputfileName, "w");
    assert(outputfile);
    int i;
    ...
    for (i = 0; i < wtCount; i++) {
        ...
        fprintf(outputfile,
                "Watchtower ID: %s, Postcode: %s, Population Served: %d, "
                "Watchtower Point of Contact Name: %s, x: %f, y: %f, Diameter "
                "of Cell: %f\n",
                (wts[i])->watchtowerID, (wts[i])->postcode,
                (wts[i])->populationServed, (wts[i])->contact, (wts[i])->x,
                (wts[i])->y, diameter);
...
void outputResultDiameterSorted(char *outputfileName,
                                struct watchtowerStruct **wts, int wtCount,
                                struct DCEL *dcel) {
    for (int i = 1; i < wtCount; i++) {
        struct watchtowerStruct *tmp = wts[i];
        double tmpDiameter = getDiameter(dcel, tmp->face);
        int j = i - 1;
        while (j >= 0 && getDiameter(dcel, wts[j]->face) > tmpDiameter) {
            wts[j + 1] = wts[j];
            j--;
        }
        wts[j + 1] = tmp;
...
char *getWTDataString(struct watchtowerStruct **wts, int wtIndex) {
    ...
    size_t requiredSpace =
        snprintf(NULL, 0,
                 "Watchtower ID: %s, Postcode: %s, Population Served: %d, "
                 "Watchtower Point of Contact Name: %s, x: %f, y: %f\n",
                 (wts[wtIndex])->watchtowerID, (wts[wtIndex])->postcode,
                 (wts[wtIndex])->populationServed, (wts[wtIndex])->contact,
                 (wts[wtIndex])->x, (wts[wtIndex])->y);
