...
void printArgError(char *error, int argCount, enum assignmentStage stage) {
    ...
    if (stage == UNSET || stage == STAGE_ERROR || stage == STAGE_0) {
        ...
        fprintf(stderr,
                "\t./voronoi2 0 datafile polygonfile outputfile < splitsfile\n"
                "\t\tTo generate the polygon in polygonfile, layout the points "
                "in \n"
                "\t\tthe datafile on the polygon, apply the splits in the "
                "splitsfile \n"
    ...
    if (stage == UNSET || stage == STAGE_ERROR || stage == STAGE_1) {
        ...
        fprintf(stderr, "\t./voronoi2 1 pointpairs outputfile\n"
                        "\t\tTo output the bisectors for the given pointpairs "
                        "to the given\n"
                        "\t\toutputfile.\n\n");
    }
    if (stage == UNSET || stage == STAGE_ERROR || stage == STAGE_2) {
        /* Mode 2: Voronoi2 Stage 2 - Print intersections between bisectors and
         * polygon. */
        fprintf(stderr, "\t./voronoi2 2 pointpairs polygonfile outputfile\n"
                        "\t\tTo output the intersections the given bisectors "
                        "make with the polygon \n"
                        "\t\tto the given outputfile.\n\n");
    }
    if (stage == UNSET || stage == STAGE_ERROR || stage == STAGE_3) {
        /* Mode 3: Voronoi2 Stage 3 - Print diameter of each face, constructing
         * using incremental method. */
        fprintf(stderr, "\t./voronoi2 3 datafile polygonfile outputfile\n"
                        "\t\tTo generate the initial polygon in polygonfile, "
                        "layout the watchtower \n"
                        "\t\tpoints in the datafile on the polygon, generate "
                        "the voronoi diagram, adding\n"
                        "\t\tmodifying the polygon so that a voronoi diagram "
                        "is constructed, seperating \n"
                        "\t\tall points into their own cell with all points in "
                        "the cell being closest to\n"
                        "\t\tthe watchtower in the cell. The watchtower data "
                        "and its diameter is\n"
    ...
    if (stage == UNSET || stage == STAGE_ERROR || stage == STAGE_4) {
        ...
        fprintf(stderr, "\t./voronoi2 4 datafile polygonfile outputfile\n"
                        "\t\tSame as stage 3, but output watchtowers are in "
                        "order of diameter, smallest \n"
                        "\t\tto largest.\n\n");
    }
    exit(EXIT_FAILURE);
}
