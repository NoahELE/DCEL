...
struct watchtowerStruct **readDataFile(char *datafileName,
                                       int *watchTowerCount) {
    assert(datafileName);
    ...
    while (getline(&line, &lineAlloced, file) != (-1)) {
        ...
        if (wtsAlloced == 0) {
            wts = (struct watchtowerStruct **) malloc(
                sizeof(struct watchtowerStruct *) * INITIALWTS);
            assert(wts);
            wtsAlloced = INITIALWTS;
        } else if ((wtsUsed + 1) > wtsAlloced) {
            wts = (struct watchtowerStruct **) realloc(
                wts, sizeof(struct watchtowerStruct *) * wtsAlloced * 2);
            assert(wts);
            wtsAlloced *= 2;
        }
        currentWT =
            (struct watchtowerStruct *) malloc(sizeof(struct watchtowerStruct));
        assert(currentWT);
        ...
        while (line[progress] != '\0') {
            ...
            if (line[progress] == ',') {
                ...
                case 0:
                    ...
                    //    malloc((strlen(line + fieldStart) + 1) *
                    //    sizeof(char));
                ...
                case 4:
                    line[progress] = '\0';
                    assert(sscanf(line + fieldStart, "%lf", &(currentWT->x)) ==
                           1);
                    fieldStart = progress + 1;
                    fieldProgress++;
                    break;
        ...
        if (line[progress] == '\0') {
            ...
            assert(sscanf(line + fieldStart, "%lf", &(currentWT->y)) == 1);
            fieldStart = progress + 1;
            fieldProgress++;
        }
